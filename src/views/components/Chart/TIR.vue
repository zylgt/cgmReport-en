...
export default {
    ...
    mounted(){
        this.$bus.$on('getDatas',(data)=>{
            this.unit = data.unit
            let new_data = _.uniqBy(data.arrayData,'DataTs')
            let v_data = _.map(new_data,'Value')
            let result =   data.unit=='mmol/L'?TIRUtils.getTIRResult(v_data,_.round( GlucoseUtils.mmolToMgdl(data.target[1]),1),_.round( GlucoseUtils.mmolToMgdl(data.target[0]),1)):TIRUtils.getTIRResult(v_data,data.target[1],data.target[0])
            ...
            this.levels[2].desc = data.target[0]+'-'+ data.target[1]+'mmol/L'
            this.levels[2].descmg = data.target[0]+'-'+data.target[1]+'mg/dL'
            this.levels[1].desc = _.round(data.target[1]+0.1,1)+'-13.9mmol/L'
            this.levels[1].descmg = data.target[1]+1+'-250mg/dL'
            this.levels[3].desc = '3.0-'+_.round(data.target[0]-0.1,1)+'mmol/L'
            this.levels[3].descmg = '54-'+(data.target[0]-1)+'mg/dL'
